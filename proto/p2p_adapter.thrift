...
struct SleepIntent {
    ...
    /**
     * Идентифкатор, по которому обработчик обратного запроса сможет идентифицировать сессию
     ...
     * Один и тот же идентифкатор можно устанавливать для одной и той же сессии до тех пор, пока
     * обратный вызов с таким идетифкатором не будет успешно обработан. Попытка установить уже
...
struct ProcessOperationInfo {
     ...
     * Сумма является просто аттрибутом операции, от которой могут вычисляться прочие величины.
     */
...
service P2PAdapterHost {
     ...
     *    идентифкатором не был обработан успешно.
     */
