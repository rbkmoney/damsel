...
service Automaton {
     ...
     *  Если передан event_id = 0, то возвращается список событий,
     *  начиная с самого первого в истории.
     *  Параметр chunk_size задает максимальную длину возвращаемой истории
     *  - если событий у машины больше, чем заданный размер, то они не войдут
     *  в возвращаемую историю.
     *
     *  Для их получения требуется осуществить дополнительные вызовы метода
     *  уже с новым after_id, соответствующим последнему вычитанному.
