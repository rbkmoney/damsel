...
service Automaton {
    ...
    /**
     * Метод возвращает список событий (историю) машины ref, начиная с события, следующего за событием after_id.
       Если передан event_id последнего события для данной машины, то возвращается пустой список.
       Если передан event_id=undefined, то возвращается список событий, начиная с самого первого в истории.
       Параметр chunk_size задает максимальную длину возвращаемой истории - если событий у машины больше,
       чем заданный размер, то они не войдут в возвращаемую историю. Для их получения требуется осуществить
       дополнительные вызовы метода уже с новым after_id, соответствующим последнему вычитанному.
       (см пример псевдокода в https://rbkmoney.atlassian.net/browse/MG-6)
       Возвращаемый список событий упорядочен по моменту фиксирования его в платежной системе:
       в начале списка располагаются события, произошедшие раньше тех, которые располагаются в конце.
       Если параметр event_id содержит некорректный (несуществующий) event, то кидается исключение EventNotFound.
       Если параметр ref содержит ссылку на несуществующую машину (несуществующий тэг или id), то кидается
       исключение MachineNotFound.
     **/
    History getHistory(1: Reference ref, 2: EventId after_id, Uint chunk_size)
         throws (1: base.MachineNotFound ex, 2: base.EventNotFound)
}
